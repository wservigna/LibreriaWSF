...
namespace WSF.Web.Localization
{
    internal class LocalizationScriptManager : ILocalizationScriptManager, ISingletonDependency
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="WSF.Web.Localization.LocalizationScriptManager"/> class.
        /// </summary>
        ...
        private string BuildAll()
        {
            ...
            script.AppendLine();
            script.AppendLine("    WSF.localization = WSF.localization || {};");
            script.AppendLine();
            script.AppendLine("    WSF.localization.currentCulture = {");
            script.AppendLine("        name: '" + currentCulture.Name + "',");
            script.AppendLine("        displayName: '" + currentCulture.DisplayName + "'");
            script.AppendLine("    };");
            script.AppendLine();
            script.Append("    WSF.localization.languages = [");
            ...
            if (languages.Count > 0)
            {
                ...
                script.AppendLine("    WSF.localization.currentLanguage = {");
            ...
            script.AppendLine();
            script.AppendLine("    WSF.localization.values = WSF.localization.values || {};");
            script.AppendLine();

            foreach (var source in _localizationManager.GetAllSources().OrderBy(s => s.Name))
            {
                script.AppendLine("    WSF.localization.values['" + source.Name + "'] = {");
