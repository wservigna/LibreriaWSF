...
namespace WSF.EntityFramework.SoftDeleting
{
    internal class SoftDeleteQueryVisitor : DefaultExpressionVisitor
    {
        public override DbExpression Visit(DbScanExpression expression)
        {
            if (!expression.Target.ElementType.MetadataProperties.Any(mp => mp.Name.EndsWith("customannotation:" + WSFEfConsts.SoftDeleteCustomAnnotationName)))
            {
