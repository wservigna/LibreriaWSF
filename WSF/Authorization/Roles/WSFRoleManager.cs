...
namespace WSF.Authorization.Roles
{
    ...
    /// </summary>
    public abstract class WSFRoleManager<TTenant, TRole, TUser> : RoleManager<TRole, int>, ITransientDependency
        ...
        where TUser : WSFUser<TTenant, TUser>
    {
        private IRolePermissionStore<TTenant, TRole, TUser> RolePermissionStore
        {
            get
            {
                ...
                {
                    throw new WSFException("Store is not IRolePermissionStore");
                }
        ...
        protected WSFRoleManager(WSFRoleStore<TTenant, TRole, TUser> store, IPermissionManager permissionManager)
            : base(store)
        {
        ...
        public override Task<IdentityResult> DeleteAsync(TRole role)
        {
            if (role.IsStatic)
            {
                throw new WSFException("Can not delete a static role: " + role);
            }
        ...
        private Permission GetPermission(string permissionName)
        {
            ...
            if (permission == null)
            {
                throw new WSFException("There is no permission with name: " + permissionName);
            }
        ...
        private async Task<TRole> GetRole(int roleId)
        {
            ...
            if (role == null)
            {
                throw new WSFException("There is no role with id = " + roleId);
            }
        ...
        private async Task<TRole> GetRole(string roleName)
        {
            ...
            if (role == null)
            {
                throw new WSFException("There is no role with name = " + roleName);
            }
