...
{
    /// <summary>
    /// Used to store WSFModuleInfo objects as a dictionary.
    /// </summary>
    internal class WSFModuleCollection : List<WSFModuleInfo>
    {
        /// <summary>
        ...
        public TModule GetModule<TModule>() where TModule : WSFModule
        {
            var module = this.FirstOrDefault(m => m.Type == typeof(TModule));
            if (module == null)
            {
                throw new WSFException("Can not find module for " + typeof(TModule).FullName);
            }
        ...
        public List<WSFModuleInfo> GetSortedModuleListByDependency()
        {
            var orderedList = new List<WSFModuleInfo>();
