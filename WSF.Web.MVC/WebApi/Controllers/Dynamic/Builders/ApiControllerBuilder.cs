...
using WSF.Logging;
using WSF.WebApi.Controllers.Dynamic.Interceptors;
using Castle.MicroKernel.Registration;
using System.Web.Http.Filters;

namespace WSF.WebApi.Controllers.Dynamic.Builders
{
    /// <summary>
    ...
    internal class ApiControllerBuilder<T> : IApiControllerBuilder<T>
    {
        ...
        public IApiControllerActionBuilder<T> ForMethod(string methodName)
        {
            ...
            {
                throw new WSFException("There is no method with name " + methodName + " in type " + typeof(T).Name);
            }
        ...
        public void Build()
        {
            ...
            IocManager.Instance.IocContainer.Register(
                Component.For<WSFDynamicApiControllerInterceptor<T>>().LifestyleTransient(),
                Component.For<DynamicApiController<T>>().Proxy.AdditionalInterfaces(new[] { typeof(T) }).Interceptors<WSFDynamicApiControllerInterceptor<T>>().LifestyleTransient()
                );
